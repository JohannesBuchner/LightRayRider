...
Author: Johannes Buchner (C) 2013-2024
License: AGPLv3

See README and LICENSE file.
"""
# Import numpy and its Cython typing helpers
import numpy as np
...
def py_sphere_raytrace_count_between(
    ...
    return sphere_raytrace_count_between(&xx[0], &yy[0], &zz[0], &RR[0], lenxx, &a[0], &b[0], &c[0], lena, &NHout[0])
...
def py_grid_raytrace(
    ...
    cdef cnp.ndarray[double, ndim=1] NHout = np.zeros(lena) - 1
    cdef int r = grid_raytrace(&rho_flat[0], lenrho, &x[0], &y[0], &z[0], &a[0], &b[0], &c[0], lena, &NHout[0])
    if r != 0:
...
def py_voronoi_raytrace(
    ...
    cdef int r = voronoi_raytrace(&xx[0], &yy[0], &zz[0], &RR[0], &rho[0], lenxx, &a[0], &b[0], &c[0], lena, &mindistances[0], lenmd, &NHout[0])
    if r != 0:
...
def py_sphere_sphere_collisions(double[::1] xx,
    ...
    cdef int r = sphere_sphere_collisions(&xx[0], &yy[0], &zz[0], &RR[0], lenxx, kstop, &NHout[0])
    if r != 0:
...
def py_sphere_raytrace_finite(
    ...
    cdef int r = sphere_raytrace_finite(&xx[0], &yy[0], &zz[0], &RR[0], &rho[0], lenxx, &x[0], &y[0], &z[0], &a[0], &b[0], &c[0], lena, &NHmax[0], &t[0])
    if r != 0:
...
def py_cone_raytrace_finite(
    ...
    assert len(d) == lena
    cdef cnp.ndarray[double, ndim=1] t = np.zeros(lena)
    cdef int r = cone_raytrace_finite(&thetas[0], &rhos[0], n, &x[0], &y[0], &z[0], &a[0], &b[0], &c[0], lena, &d[0], &t[0])
    if r != 0:
...
def py_grid_raytrace_finite(
    ...
    assert len(z) == lena
    cdef cnp.ndarray[double, ndim=1] t = np.zeros(lena)
    cdef int r = grid_raytrace_finite(&rho_flat[0], lenrho, &x[0], &y[0], &z[0], &a[0], &b[0], &c[0], lena, &NHmax[0], &t[0])
    if r != 0:
